...
Sin `SUPABASE_SERVICE_ROLE_KEY` las evidencias se guardan en `.data/evidencias/` (solo para desarrollo). Para una prueba completa con navegador: `node tests/e2e/crear-pedido.mjs`, luego `node tests/e2e/flujo.mjs` `node tests/e2e/reparto.mjs` (reparto por actividad), `node tests/e2e/fin-evento.mjs` (fin de evento y colores) y, para el bot, `node tests/e2e/telegram.mjs` (requieren la app corriendo y Chromium de Playwright).
